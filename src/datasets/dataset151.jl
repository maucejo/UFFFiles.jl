...
function parse_dataset151(io)
    # Record 1 to 3 - FORMAT(80A1)
    model_name = strip(readline(io))
    description = strip(readline(io))
    application = strip(readline(io))

    # Record 4 - FORMAT(10A1,10A1,3I10)
    # Field 1 & 2: date and time created
    line_creation = extend_line(readline(io))
    datetime_created = strip(line_creation[1:20])

    # Record 4 - Field 3
    tmp = strip(line_creation[21:30])
    version = isempty(tmp) ? 0 : parse(Int, tmp)

    # Record 4 - Field 4
    tmp = strip(line_creation[31:40])
    version2 = isempty(tmp) ? 0 : parse(Int, tmp)

    # Record 4 - Field 5
    tmp = strip(line_creation[41:50])
    file_type = isempty(tmp) ? 0 : parse(Int, tmp)

    # Record 5 - FORMAT(10A1,10A1)
    datetime_last_saved = strip(readline(io))

    # Record 6 - FORMAT(80A1)
    program = strip(readline(io))

    # Record 7 - FORMAT(10A1,10A1)
    datetime_written = strip(readline(io))
